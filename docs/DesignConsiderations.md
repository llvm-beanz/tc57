...
HLSL seeks to be a powerful portable data parallel programming language that
enables high performance applications to target data parallel hardware across a
wide variety of end user devices. HLSL is not limited to Microsoft hardware and
...
map efficiently to data parallel hardware (such as `virtual`, RTTI, and
exceptions), we should strive for alignment with C++ wherever possible and
